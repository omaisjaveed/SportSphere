...
class CheckoutController extends Controller
{
    ...
    public function verified_user_payment(Request $request, $user_id){
        // $user_id = decrypt($user_id);
        $user = User::find($user_id);
        
        // dd($order);
        
        if($user->is_verified == 'NULL' || $user->is_verified == NULL){
            return view("theme.$this->theme.verified_user_payment",compact('user'));
        }

        return back();  
    }
    
    public function premium_user_payment(Request $request, $user_id){
        // $user_id = decrypt($user_id);
        $user = User::find($user_id);
        
        // dd($order);
        
        if($user->is_premium == 'NULL' || $user->is_premium == NULL){
            return view("theme.$this->theme.premium_user_payment",compact('user'));
        }
