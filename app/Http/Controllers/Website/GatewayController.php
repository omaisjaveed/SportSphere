...
class GatewayController extends Controller {
	...
	public function stripe_payment_authorize(Request $request, $user_id) {
        @ini_set('max_execution_time', 0);
        @set_time_limit(0);
        ...
        // ✅ For verified user
        if ($request->payment_type_verified == 'verified_user') {
            $charge = Stripe\Charge::create([
                "amount" => 9.99 * 100,
                "currency" => 'usd',
                "source" => $request->stripeToken,
                "description" => "Verified User Payment",
            ]);
            ...
            if ($charge->amount_refunded == 0 && $charge->failure_code == null && $charge->paid == true && $charge->captured == true) {
                ...
                return view("theme.$this->theme.verified-user-payment-complete", compact('userObj'));
            } else {
                return back()->with('error', _lang('Sorry, Payment not completed, Please contact with your administrator!'));
            }
        }
        ...
        // ✅ For premium user
        if ($request->payment_type_premium == 'premium_user') {
            $charge = Stripe\Charge::create([
                "amount" => 9.99 * 100,
                "currency" => 'usd',
                "source" => $request->stripeToken,
            ...
            if ($charge->amount_refunded == 0 && $charge->failure_code == null && $charge->paid == true && $charge->captured == true) {
                ...
                $user->premium_expires_at = now()->addMonth(); // add 30 days
                $user->save();
                ...
                return view("theme.$this->theme.premium-user-payment-complete", compact('userObj'));
            } else {
                return back()->with('error', _lang('Sorry, Payment not completed, Please contact with your administrator!'));
            }
        }
...
//             Stripe\Stripe::setApiKey(get_option('stripe_secret_key'));
//             $charge = Stripe\Charge::create([
//                 "amount" => 9.99 * 100, // $80 in cents
//                 "currency" => 'usd',
//                 "source" => $request->stripeToken,
//                 "description" => "Tryout Payment",
//             ]);
