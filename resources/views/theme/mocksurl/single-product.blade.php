					...
					<div class="col-lg-6 col-12">
						...
						<div class="product-gallery">
							...
							<div class="flexslider-thumbnails">
								<ul class="slides">
									<li data-thumb="{{ asset('storage/app/' . $product->image->file_path) }}"
						...
						<div class="product-des">
							...
							<div class="short">
								...
								<div class="rating-main">
									...
									<a href="#" class="total-review">({{ $product->reviews->count() }})
										{{ _lang('Review') }}</a>
								</div>
								...
								@else
									<p class="price">
										@if($product->variation_prices[0]->special_price != '' || (int) $product->variation_prices[0]->special_price != 0)
											<span
												class="discount">{!! xss_clean(show_price($product->variation_prices[0]->special_price)) !!}</span>
											<s>{!! xss_clean(show_price($product->variation_prices[0]->price)) !!}</s>
										@else
											<span
												class="discount">{!! xss_clean(show_price($product->variation_prices[0]->price)) !!}</span>
										@endif
									</p>
								@endif
							...
							<div class="product-buy">
								...
								<div class="add-to-cart">
									@if($product->in_stock == 1)
										<a href="{{ url('add_to_cart/' . $product->id) }}"
											...
											class="btn add_to_cart">{{ _lang('Add to Cart') }}</a>
									@else
										<a href="#" class="btn disabled">{{ _lang('Add to Cart') }}</a>
									@endif
									<a href="{{ wishlist_url($product) }}" class="btn min btn-wishlist"><i
											class="ti-heart"></i></a>
								</div>



								<!-- <p class="cat">{{ _lang('Category') }} :
								...
								@if($product->manage_stock == 1 && $product->in_stock == 1)
									<p class="availability">{{ _lang('Availability') }} :
										{{ $product->qty . ' ' . _lang('Product In Stock') }}
									</p>
								@endif
							...
							<div class="nav-main">
								...
								<ul class="nav nav-tabs" id="myTab" role="tablist">
									<li class="nav-item"><a class="nav-link active" data-toggle="tab"
											href="#description" role="tab">{{ _lang('Description') }}</a></li>
									<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#reviews"
											role="tab">{{ _lang('Reviews') }} ({{ $product->reviews->count() }})</a>
									</li>
									<li class="nav-item"><a class="nav-link" data-toggle="tab" href="#comments"
											role="tab">{{ _lang('Comments') }} ({{ $product->comments->count() }})</a>
									</li>
								</ul>
								<!--/ End Tab Nav -->
							</div>
											...
											<div class="col-12">
												<div class="ratting-main">
													...
													<div class="avg-ratting">
														<h5><span
																class="total-reviews">{{ $product->reviews->count() }}</span>
															{{ _lang('Reviews for') . ' ' . $product->translation->name }}
														</h5>
													</div>
													...
													@foreach($product->reviews as $review)
														...
														<div class="single-rating">
															<div class="rating-author">
																<img src="{{ profile_picture($review->user->profile_picture) }}"
																	alt="{{ _lang('Profile Picture') }}">
															</div>
												...
												<div class="comment-review">
													<div class="add-review">
														...
														@if(!auth()->check())
															<p>{{ _lang('You need to login for writing a review.') }} <a
																	href="{{ url('sign_in') }}"
																	class="btn-link">{{ _lang('Login to your account') }}</a>
															</p>
														@endif
													</div>
												...
												<!--/ End Review -->
												<!-- Form -->
												<form class="form reviews-form" method="post"
								...
								<div class="tab-pane fade" id="comments" role="tabpanel">
									<div class="tab-single">
										<div class="row">
											<div class="col-12">
												<div class="comments">
													<h3 class="comment-title">{{ _lang('Comments') }} (<span
															class="comments-count">{{ $product->comments->count() }}</span>)
													</h3>
											...
											<div class="col-12">
												<div class="reply">
													<div class="reply-head">
														...
														@if(!auth()->check())
															<p>{{ _lang('You need login for posting a comment.') }} <a
																	href="{{ url('sign_in') }}"
																	class="btn-link">{{ _lang('Login to your account') }}</a>
															</p>
