							...
							<div class="row">
								...
								<div class="col-12 mb-3">
                                    <div class="form-group create-account">
                                        <label>
                                            <a href="{{ url('/sign-up') }}" id="create_account_link" class="create-account-link">
										...
										<select name="shipping_address" id="select-shipping-address" required>
											@foreach($address_list as $shipping_address)
												<option value="{{ $shipping_address->id }}"
													data-state="{{ $shipping_address->state }}" {{ $shipping_address->is_default == 1 ? 'selected' : '' }}>
										...
										<select name="billing_address" id="select-billing-address" required>
											@foreach($address_list as $billing_address)
												<option value="{{ $billing_address->id }}"
													data-state="{{ $billing_address->state }}" {{ $billing_address->is_default == 1 ? 'selected' : '' }}>
							...
							<form action="{{ url('/add_new_address') }}" id="create_address_form" method="post">
								...
								<div class="row">
									...
									<div class="col-md-6">
										...
										<select class="form-control customer-country" data-selected="{{ old('country') }}"
									...
									<div class="col-md-12">
										<textarea class="form-control" name="address" placeholder="{{ _lang('Address') }}"
				...
				<div class="order-details">
					...
					<div class="single-widget">
						...
						<div class="content" id="cart-contents">
							@include('theme.mocksurl.components.checkout-cart')
						</div>
					</div>
					<!--/ End Order Widget -->
					...
					<!--					@csrf-->
					<!--					<input type="text" name="coupon" placeholder="{{ _lang('Enter Coupon Code') }}"-->
					<!--						required>-->
							...
							@if(!Cart::isEmpty())
								<ul>
										...
										<li>
											<div class="custom-control custom-radio">
												<input type="radio" id="free_shipping" value="free_shipping"
													name="shipping_method" class="custom-control-input select-shipping-method"
									...
									@if(get_option('flat_rate_active') == 'Yes')
										<li>
											<div class="custom-control custom-radio">
												<input type="radio" id="flat_rate" value="flat_rate" name="shipping_method"
													class="custom-control-input select-shipping-method" {{ empty(Cart::getCondition(get_option('flat_rate_label'))) ? '' : 'checked' }}>
												<label class="custom-control-label"
