					...
					<div class="single-widget range">
						...
						<div class="price-filter">
							<div class="price-filter-inner">
								<div id="slider-range" data-min="0" data-max="{{ get_max_price() }}"
									data-value="{{ isset($_GET['end_price']) ? $_GET['end_price'] : get_max_price() }}">
								</div>

								<div class="price_slider_amount">
									<div class="label-input">
										<input type="text" id="amount" class="text-center" name="price"
											placeholder="{{ _lang('Add Your Price') }}" />
										<input type="hidden" id="start_price" value="0" class="filter" />
										<input type="hidden" id="end_price"
											value="{{ isset($_GET['end_price']) ? $_GET['end_price'] : get_max_price() }}"
											class="filter" />
									</div>
								</div>
							</div>
							<button type="button" class="btn btn-block py-2 mt-2"
								id="filter_by_price">{{ _lang('Filter') }}</button>
						</div>

					</div>
					<div class="content">
					    <p>Full90 merch is custom made with ‘90’ creative in mind.  If you have a creative mind and want something beyond what you see, contact us, we’ll hook it up! <span>Trust us, “we’ll hook it up”.</span>  Personalize with your name - contact us!</p>
					</div>
					<!--/ End Shop By Price -->
								...
								<div class="single-shorter">
									...
									<select id="sort_by" class="filter">
										...
										<option value="newest" {{ $sort_by == "newest" ? "selected" : "" }}>
											{{ _lang('Newest') }}</option>
										<option value="low_to_high" {{ $sort_by == "low_to_high" ? "selected" : "" }}>
											{{ _lang('Price: Low to High') }}</option>
										<option value="high_to_low" {{ $sort_by == "high_to_low" ? "selected" : "" }}>
											{{ _lang('Price: High to Low') }}</option>
										<option value="a_to_z" {{ $sort_by == "a_to_z" ? "selected" : "" }}>
											{{ _lang('Name: A-Z') }}</option>
										<option value="z_to_a" {{ $sort_by == "z_to_a" ? "selected" : "" }}>
											{{ _lang('Name: Z-A') }}</option>
									</select>
								</div>
							</div>
						</div>
